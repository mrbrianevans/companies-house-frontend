...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    const { rows: filingEvents } = await pool.query("SELECT * FROM filing_events WHERE company_number=$1;", [company_number]);
    console.timeEnd("Query database SELECT filing events");
    console.time("Query database SELECT company events");
    const { rows: companyEvents } = await pool.query("SELECT * FROM company_events WHERE company_number=$1;", [company_number]);
