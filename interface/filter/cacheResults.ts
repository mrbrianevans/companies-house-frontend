...
export async function cacheResults<FilterResultsType>({
  ...
  const { rows: preexistingResults }: { rows: FilterResultsType[] } = await pool.query(
    `
      ...
      FROM cached_filter_results cfr 
           LEFT JOIN ${config.main_table} m ON cfr.data_fk = m.${config.uniqueIdentifier}
  ...
  if (preexistingResults.length === 0) {
    ...
    const { results } = await applyFilters<FilterResultsType>({ filters, category, limit: qty ?? 20 })
  ...
  `,
    ...
    timer.flush()
    return { results: preexistingResults }
  }
}
