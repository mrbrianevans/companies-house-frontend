 ...
 */
async function getCachedFilter<FilterResultsType>({ cachedFilterId }: GetCachedFilterParams): Promise<ISavedFilter<FilterResultsType> | null> {
  ...
  const cachedFilter:ISavedFilter<FilterResultsType> = {
    ...
    metadata: {
      id: cachedFilterId,
      lastRunTime: rows[0].time_to_run[rows[0].time_to_run.length-1],
